...
const wrap = (pos) => {
  ...
  const [i, direction] = DICE_SQUARE.find(
    DICE_SQUARE => DICE_SQUARE.x === pos.x - offset.x && DICE_SQUARE.y === pos.y - offset.y,
  ).wrap[pos.direction];
  ...
  return { x: DICE_SQUARE[i].x + next.x, y: DICE_SQUARE[i].y + next.y, direction };
};
