...
const possiblePaths = validValveIds.reduce(
  (paths, id) => paths.concat(paths.map(set => [...set, id])),
  [[]]
).slice(1);

const validPathPairs = [];
for (let i = 0; i < possiblePaths.length; i++) {
  const j = possiblePaths.findIndex((path) =>
...
const results = validPathPairs.map(([human, elephant]) => 
