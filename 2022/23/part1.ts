...
const move = (elves: Set<string>, proposals: Map<string, string[]>): void => {
...
const round = (elves: Set<string>, checks: string[]): void => {
  const proposals = new Map();
  for (const key of elves.keys()) {
    const all = checks.flatMap((check) => getOptions(check, key));
    if (all.every((option) => !elves.has(option))) {
      // do nothing if no other Elves in surrounding
      continue;
    }
...
const result =
  (xs.at(0)! - xs.at(-1)! + 1) * (ys.at(0)! - ys.at(-1)! + 1) - elves.size;
