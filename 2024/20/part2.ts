...
const race = (maze: { x: number, y: number, isWall: boolean }[][], start: { x: number, y: number }, end: { x: number, y: number }, stopIfOver = undefined) => {
...
originalTrack.forEach((cheatStart, startIndex) => {
	...
	originalTrack.forEach((cheatEnd, endIndex) => {
		...
		if (cheatCount <= 1 || cheatCount > maxCheatCount)
			return;

		if (originalTrack[startIndex + cheatCount] === cheatEnd) // Ignore cheat that's already on track
			return;
