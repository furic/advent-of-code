...
function findCombinations(row, groups) {
    ...
    return cache[line] ??= (row[0] != "#" ? findCombinations(trimStart(row.slice(1)), groups) : 0) +
        (isFirstGroupDamagedOrUnknown && row[firstGroup] != "#" ? findCombinations(trimStart(row.slice(firstGroup + 1)), groups.slice(1)) : 0);
}

for (const line of input) {
    const [row, groups] = line.split(" ").map((x, i) => i == 1 ? x.split(",").map(y => Number(y)) : x);
