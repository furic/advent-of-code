...
for (const connection of importantConnections) {
  components[connection[0]].splice(
    components[connection[0]].indexOf(connection[1]),
    1,
  );
  if (components[connection[0]].length <= 0) {
    delete components[connection[0]];
  }
  components[connection[1]].splice(
    components[connection[1]].indexOf(connection[0]),
    1,
  );
  if (components[connection[1]].length <= 0) {
    delete components[connection[1]];
  }
}
