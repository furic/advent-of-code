...
for (const str of input) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    for (let i = r - 1, j = r; i >= 0 && j < rows.length; i--, j++) {
      if (rows[i] !== rows[j]) {
        failedInstances.push([i, j]);
      }
    }

    if (
      failedInstances.length != 1 ||
      rows[failedInstances[0][0]]
        .split("")
        .map((x, i) => x != rows[failedInstances[0][1]][i])
        .filter((x) => x).length != 1
    ) {
  ...
  for (let c = 1; c < columns.length; c++) {
    ...
    if (
      failedInstances.length != 1 ||
      columns[failedInstances[0][0]]
        .split("")
        .map((x, i) => x != columns[failedInstances[0][1]][i])
        .filter((x) => x).length != 1
    ) {
