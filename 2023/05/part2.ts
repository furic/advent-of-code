...
function expand(index, values) {
  ...
  for (
    ;
    nextMapIndex < maps.length && maps[nextMapIndex].length === 3;
    nextMapIndex++
  );
  ...
  if (
    ...
    result.push(
      ...expand(nextMapIndex, firstTuple),
    ...
    result.push(
      ...expand(nextMapIndex, [destination + values[0] - source, values[1]]),
    );
  } else if (values[0] < source && values[0] + values[1] > source + range) {
